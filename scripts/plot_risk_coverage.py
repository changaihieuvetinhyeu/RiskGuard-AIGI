#!/usr/bin/env python3
"""Create Figure 2 from the disjoint SAFE held-out artifacts."""
...
CLEAN = ROOT / "artifacts" / "protocol_clean_v2"
FIG_DIR = ROOT / "reports" / "protocol_clean_v2" / "figures"

THRESHOLD_REGISTRY = CLEAN / "certification" / "certification_registry.csv"
FINAL_METRICS = CLEAN / "final_metrics" / "selective_metrics.csv"
...
def load_curve_data() -> pd.DataFrame:
    ...
    for split in SPLITS:
        sources = {
            method: (CLEAN / "scores" / DETECTOR / split / method / f"{DATASET}.parquet", "risk_score")
...
def make_figure(curves: pd.DataFrame, points: pd.DataFrame) -> dict[str, Any]:
    ...
    for ax, split in zip(axes, SPLITS, strict=True):
        ...
        ax.set_yticks([0.00, 0.02, 0.04, 0.05, 0.06])
        ax.set_yticklabels(["0.00", "0.02", "0.04", "0.05", "0.06"])
...
def write_audit(
    ...
    filters = {
        "official_riskguard_scores": {
            "paths": [f"artifacts/protocol_clean_v2/scores/{DETECTOR}/{split}/riskguard/{DATASET}.parquet" for split in SPLITS],
            "method": OFFICIAL_METHOD,
            "risk_column": "risk_score",
        },
        "baseline_scores": {
            "paths": [
                f"artifacts/protocol_clean_v2/scores/{DETECTOR}/{split}/{method}/{DATASET}.parquet"
...
- PDF: `reports/protocol_clean_v2/figures/figure2_safe_heldout_risk_coverage.pdf`
- PNG: `reports/protocol_clean_v2/figures/figure2_safe_heldout_risk_coverage.png`
- Plot data: `reports/protocol_clean_v2/figures/figure2_safe_heldout_plot_data.csv`
...
Official RiskGuard curve sources: `artifacts/protocol_clean_v2/scores/safe/{{split}}/riskguard/protocol_held_out.parquet`

Baseline curve sources: `artifacts/protocol_clean_v2/scores/safe/{{split}}/{{msp,knn}}/protocol_held_out.parquet`
...
All curves were recomputed exactly by sorting each matched clean-v2 score artifact
from lowest to highest estimated risk, with `sample_id` as the deterministic
tie-breaker. The three methods use the same SHA-disjoint held-out rows within each split.
...
- Horizontal target line: alpha = `{ALPHA}`; `0.05` is a true y-axis tick between `0.04` and `0.06`
...
- Figure canvas chosen to preserve the existing manuscript Figure 2 export footprint
- Export: vector PDF and 400 dpi PNG with tight bounding box and 0.02-inch padding
- Intended LaTeX placement: `width=\\linewidth`
...
- SAFE Split A held-out coverage ~= 0.834353 and selective risk ~= 0.001918.
- SAFE Split B held-out coverage ~= 0.904281 and selective risk ~= 0.031326.
